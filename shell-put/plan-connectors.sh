...
# PLAN:  커넥터별 설정 생성 및 명령어 출력, 최종출력에 grep -v EOF 사용시 JSON만 추출 가능
while read -r PAYLOAD; do

  NAME=$(echo "$PAYLOAD" | yq -r '.name')
  PAYLOAD_PRETTY=$(echo "$PAYLOAD" | yq -o=json)

  cat <<PRINT_CMD
cat <<EOF | curl -s -X PUT -H 'Content-Type: application/json' -d @- '$API/$NAME/config'
$PAYLOAD_PRETTY
EOF
PRINT_CMD
